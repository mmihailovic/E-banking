...
public class Seeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        if(paymentCodeRepository.findAll().isEmpty()) {
            paymentCodeRepository.saveAll(readAndProcessFile());
        }
        if(roleRepository.findAll().isEmpty()) {
            List<Role> roles = generateRoles();
            roles = roleRepository.saveAll(roles);
            Company company = insertCompany();
            insertEmployee(roles.subList(1, roles.size()), company);
            insertClient(roles.get(0));
        }
    }

    private List<Role> generateRoles() {
        String[] roles = {"ROLE_CLIENT", "ROLE_MANAGE_CLIENTS", "ROLE_MANAGE_EMPLOYEES", "ROLE_MANAGE_COMPANIES", "ROLE_MANAGE_CARDS",
        "ROLE_MANAGE_BANK_ACCOUNTS", "ROLE_MANAGE_CREDITS", "ROLE_MANAGE_ORDERS", "ROLE_MANAGE_LISTINGS"};
    ...
    private Company insertCompany() {
        ...
        company.setName("Bank");
        company.setAccountNumbers("123");

        return companyRepository.save(company);
    }

    private void insertEmployee(List<Role> roles, Company company) {
        Employee user = new Employee();
        user.setEmail("employee@email.com");
    ...
    private void insertClient(Role clientRole) {
        Client user = new Client();
        user.setEmail("client@email.com");
        user.setPassword(bCryptPasswordEncoder.encode("123"));
        user.setActive(true);
        user.setRoles(List.of(clientRole));
        user.setAccountNumbers("456");

        userRepository.save(user);
    }

    @GeneratedCrudOperation
    public List<PaymentCode> readAndProcessFile() {
        List<PaymentCode> data = new ArrayList<>();
        HashMap<Integer,String> codes = new HashMap<>();
        full(codes);

        for(Integer code : codes.keySet()){
            data.add(new PaymentCode(null, code, codes.get(code)));
        }
        return data;
    }

    @GeneratedCrudOperation
    private void full(HashMap<Integer,String> dataMap) {
